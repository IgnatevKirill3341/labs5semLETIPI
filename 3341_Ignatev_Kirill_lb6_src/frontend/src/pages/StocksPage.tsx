...
const StocksPage: React.FC = () => {
  ...
  const prepareChartData = (historicalData: StockPrice[]) => {
    // Используем все данные, не ограничиваем 365 днями
    const labels = historicalData.map(d => {
      const [month, day, year] = d.date.split('/');
      return `${day}.${month}.${year}`;
    });
    const data = historicalData.map(d => parseFloat(d.open.replace('$', '')));
              ...
              {tabValue === 1 && (
                <TableContainer sx={{ maxHeight: { xs: '300px', sm: '400px' }, mt: 2, overflowX: 'auto' }}>
                  <Table stickyHeader size="small">
                    ...
                    <TableBody>
                      {selectedStock.historicalData.slice().reverse().map((data, index) => (
                        <TableRow key={index}>
